...
const saveSignatureFields = asyncHandler(async (req, res) => {
    ...
    if (!Array.isArray(signatureFields)) {
        throw new ApiError(400, "signatureFields must be an array");
    }

    await SignatureField.deleteMany({ documentId });
...
const getSignatureFields = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, fields, "Signature fields retrieved successfully")
    );
});
